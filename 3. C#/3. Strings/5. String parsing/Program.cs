...
namespace Parsing
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (succeeded)
            {
                Console.WriteLine($"Try parse numStr1: {targetNum}");
            }

            // Additional: Passing NumberStyles to TryParse
            succeeded = false;
            succeeded = Int32.TryParse(numStr4, NumberStyles.Float | NumberStyles.AllowThousands,
            CultureInfo.InvariantCulture, out targetNum);
            if (succeeded)
            {
                Console.WriteLine($"Try parse numStr4 with formatting: {targetNum}");
            }
